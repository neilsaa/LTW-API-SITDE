...
@app.route('/')
def index():
  return '<h1>23_Nguyen Le Minh_003</h1>'
...
def add_Employee():
  ...
  if EmployeeName and EmailAddress and AccountName and Password and Tel and DepartmentID and RoleID:
    cur.execute('INSERT INTO Employee (EmployeeName, AccountName, EmailAddress, Password, Tel, DepartmentID, RoleID) VALUES (?, ?, ?, ?, ?, ?, ?)', (EmployeeName, AccountName, EmailAddress, Password, Tel, DepartmentID, RoleID))
    conn.commit()
...
def update_Employee(id):
  ...
  RoleID = request.json.get('RoleID')
  if EmployeeName and EmailAddress and AccountName and Password and Tel and DepartmentID and RoleID:
    cur.execute('UPDATE Employee SET EmployeeName = ?, AccountName = ?, EmailAddress = ?, Password = ?, Tel = ?, DepartmentID = ?, RoleID = ? WHERE EmployeeID = ?', (EmployeeName, AccountName, EmailAddress, Password, Tel, DepartmentID, RoleID, id))
    conn.commit()
    if cur.rowcount > 0:
...
def search_Employee(email, password):
  ...
  sqlcommand = ("select * from Employee where EmailAddress like '%" + email+"%' and Password like '%" + password+  "%'")
  # Thực thi câu lệnh
...
def search(searchInput):
  ...
  cur = conn.cursor()
  # Viết câu lệnh để lấy thông tin từ database
  sqlcommand = ("select * from Employee where EmployeeName like '%" + searchInput+"%' or AccountName like '%" + searchInput+  "%' or EmailAddress like '%" + searchInput+  "%'")  
  cur.execute(sqlcommand) 
  employees = cur.fetchall()
  employees_list = []
